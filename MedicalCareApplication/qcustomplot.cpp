...
/*! \class QCPAbstractItem
  ...
  anchor idDoctor (an integer enumerating all anchors on the item, you may create an own enum for this).
...
/*! \internal
  ...
  For example, if the item has two anchors with idDoctor 0 and 1, this function takes one of these anchor
...
/*! \internal
  ...
  enum (e.g. "AnchorIndex") for this on each item that uses anchors. This idDoctor is used by the anchor
  to identify itself when it calls QCPAbstractItem::anchorPixelPoint. That function then returns
  the correct pixel coordinates for the passed anchor idDoctor.
